...
首次启动会弹出数据目录指引；也可通过「帮助 → 用户数据目录」随时查看/打开。
迁移：退出后复制整个 `data` 目录即可。
...
- 如需关闭窗口直接退出，在 `data\launcher.json` 配置：
  ```json
  { "closeToTray": false }
  ```
...
1. **配置文件**：在用户数据目录（`data\`）下创建 `launcher.json`：
   ```json
   { "port": 8080 }
   ```
...
3. **配置文件**：`data\launcher.json` 加 `remoteUrl`：
   ```json
